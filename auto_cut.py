#!/usr/bin/env python3
"""
auto_cut.py - Motion + Audio Detection Video Cutter

This script is now a thin wrapper around core.motion_detector.
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Motion + Audio Detection Video Cutter"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help=f"Motion threshold (default: {DEFAULTS['motion_threshold']})"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help=f"Audio threshold (default: {DEFAULTS['audio_threshold']})"
    )
...
def main():
    ...
    # Print parameters
    print("=" * 60)
    print("Motion + Audio Detection Video Cutter")
    print("=" * 60)
    print()
    ...
    print()
    print("Parameters:")
    print(f"  - SAMPLE_FPS: {config.motion.sample_fps}")
    print(f"  - MOTION_THRESHOLD: {config.motion.threshold}")
    print(f"  - AUDIO_THRESHOLD: {config.motion.audio_threshold}")
    print(f"  - MIN_DURATION: {config.motion.min_duration}s")
    print(f"  - MERGE_GAP: {config.motion.merge_gap}s")
    print()
